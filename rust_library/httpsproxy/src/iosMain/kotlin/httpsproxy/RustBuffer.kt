...
actual typealias RustBuffer = CValue<httpsproxy.cinterop.RustBuffer>
@Suppress("ACTUAL_WITHOUT_EXPECT", "ACTUAL_TYPE_ALIAS_WITH_COMPLEX_SUBSTITUTION")
actual typealias RustBufferPointer = CPointer<httpsproxy.cinterop.RustBuffer>
...
actual fun RustBuffer.free(): Unit =
    rustCall { status ->
        UniFFILib.ffi_httpsproxy_26fe_rustbuffer_free(this, status)
    }

actual fun allocRustBuffer(buffer: Buffer): RustBuffer =
    rustCall { status ->
        val size = buffer.size
        UniFFILib.ffi_httpsproxy_26fe_rustbuffer_alloc(size.toInt(), status).also {
            it.useContents {
