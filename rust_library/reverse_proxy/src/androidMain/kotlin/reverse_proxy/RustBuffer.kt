...
actual fun RustBuffer.free() =
    rustCall { status ->
        UniFFILib.ffi_reverse_proxy_2e6f_rustbuffer_free(this, status)
    }
...
actual fun allocRustBuffer(buffer: Buffer): RustBuffer =
    rustCall { status ->
        ...
        var readPosition = 0L
        UniFFILib.ffi_reverse_proxy_2e6f_rustbuffer_alloc(size.toInt(), status).also { rustBuffer: RustBuffer ->
